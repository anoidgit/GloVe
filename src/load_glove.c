...
char *loadvec_file; // ano add this, file to load pre-defined vectors
...
void initialize_parameters() {
	...
	// ano change this block to support load from pre-defined vector file.
	if (loadvec_file[0]=='\0'){
		for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size; a++) W[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;
	}
	else{
		FILE * fvec = fopen(loadvec_file, "r"); // ano add this
		real cur_weight = 0.0;
		for (b = 0; b < vector_size; b++) for (a = 0; a < vocab_size; a++) {
			fscanf(fvec, "%lf", &cur_weight);
			W[a * vector_size + b] = cur_weight;
			W[(a + vocab_size) * vector_size + b] = cur_weight; // ano add this, but not sure whether we should the context vector with the pre-defined vector or just randomly.
		}
		fclose(fvec);
	}
	// ano ended change the block
...
int main(int argc, char **argv) {
	...
	loadvec_file = malloc(sizeof(char) * MAX_STRING_LENGTH); // ano add this, allocate memory for loadvec_file
	int result = 0;
	...
	printf("\t\tIf 1, write vocab_size/vector_size as first line. Do nothing if 0 (default).\n");
		...
		printf("\t-load-vecfile <file>\n");// ano add this
		printf("\t\tFile containing pre-defined vectors (extract by extvec.py with vocab.txt produced by 'vocab_count'); default is none, vectors will be randomly initialized. Be caution with this, for we just initialize the context vector by the pre-defined vector.\n");
		printf("\nExample usage:\n");
	...
	if ((i = find_arg((char *)"-write-header", argc, argv)) > 0) write_header = atoi(argv[i + 1]);
		...
		if ((i = find_arg((char *)"-load-vecfile", argc, argv)) > 0) strcpy(loadvec_file, argv[i + 1]);
		else strcpy(loadvec_file, (char *)"");
